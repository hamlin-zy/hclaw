 ...
 * 交接总结直接写进 handoffSummary 参数，不要在回复正文里先输出一遍全文
 * （同一份总结输出两遍 = 白烧一遍输出 token）。
...
const inputSchema = z.object({
    ...
    handoffSummary: z.string()
        .min(1, '交接总结不能为空')
        .describe(
            '交接总结全文，将作为新会话的首条用户消息（直接写进本参数，不要在回复正文里重复输出）。必须包含以下结构：\n'
            + '## 任务目标（一句话描述当前任务）\n'
            + '## 已完成进度（关键里程碑，引用文件/命令）\n'
...
export const sessionHandoffTool: Tool<SessionHandoffInput, string> = {
    ...
    description:
        '当当前会话上下文过长、LLM 智力下降时使用。'
        + '交接总结直接写进 handoffSummary 参数：不要在调用前于回复正文里先输出一遍总结全文'
        + '（同一份总结输出两遍 = 白烧一遍输出 token），正文只写一行交接说明。'
        + '工具将创建新的独立会话，将交接总结作为新会话首条用户消息写入，'
        + '并在新会话中自动启动 Agent 从总结处继续工作。',
    inputSchema,
    isDestructive: false,

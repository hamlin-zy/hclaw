...
export function formatShortcut(shortcut: string): string {
    return mapShortcut(shortcut, key => PLATFORM_KEY[key] ?? (IS_MAC ? MAC_MODIFIER[key]?.[0] ?? key : key))
}
...
export function formatShortcutSpoken(shortcut: string): string {
    return mapShortcut(shortcut, key => SPOKEN_KEY[key] ?? (IS_MAC ? MAC_MODIFIER[key]?.[1] ?? key : key))
}

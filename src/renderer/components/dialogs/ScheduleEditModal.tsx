                ...
                <div className="px-5 py-4 space-y-3 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <ThemedSelect
                            ...
                            options={[
                                {value: '', label: '默认工作目录'},
                                ...workspaces.map(ws => ({value: ws.id, label: `${ws.name} (${ws.path})`})),
                            ]}
                            ariaLabel="工作目录"
                        />
                                    ...
                                    {cron.mode === 'monthly' && (
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2 text-xs flex-wrap">
                                                <span>每月</span>
                                                <ThemedSelect
                                                    value={String(cron.monthlyDate)}
                                                    onChange={v => updateCron({monthlyDate: parseInt(v)})}
                                                    options={MONTHLY_DATES.map(d => ({value: String(d), label: `${d} 日`}))}
                                                    ariaLabel="每月日期"
                                                />
                                            </div>
                                            <div className="flex items-center gap-2 text-xs">
                                    ...
                                    {cron.mode === 'interval' && (
                                        <div className="flex items-center gap-2 text-xs">
                                            ...
                                            <ThemedSelect
                                                ...
                                                options={[{value: 'minutes', label: '分钟'}, {value: 'hours', label: '小时'}]}
                                                ariaLabel="间隔单位"
                                            />
                                        </div>
                                    )}

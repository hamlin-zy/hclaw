...
- **谨慎 ≠ 不委派** — 凡有能够匹配的专职 agent，应把认知型任务交出去，主会话只保留判断与调度；"最小变更"约束的是最终代码改动，不是这里的协作方式。`,
    },
    {
        key: 'system.workflow',
        name: '工作流程',
...
- **高效更新** — 增量修改时简短说明变更即可
- **思考不预写正文** — 思考只用于推理决策；不要在其中完整预写最终正文再誊抄一遍，正文一次成稿（同一内容生成两遍会白烧一遍输出 token，且思考内容随历史回传、后续每轮再按输入计费一次）`,
    },
    {
        key: 'system.routing',
        name: '任务分发协议',

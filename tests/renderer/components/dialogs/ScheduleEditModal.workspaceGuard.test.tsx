...
/**
 * 定时任务编辑弹窗 · 工作目录必填、失效占位与「列表未就绪不得放行」（票 11 · workspace-guard）
 *
 * 口径：渲染真实弹窗，断言**用户看到什么、点了会发生什么**。
 *   1. 下拉里不再有「默认工作目录」这一项（旧的可选项会把任务落到非注册目录上）；
 *   2. 当前值解析不到现存工作区时，以占位项把原值摆出来（已失效 / 未设置 / 待校验）；
 *   3. 保存被拦：原因可读、不关窗、表单不丢；改成有效工作目录即可正常保存。
 *
...
/**
 * 打开工作目录下拉并等它真的渲染出来（选项渲染 = 工作区列表已取回）。
...
/** 工作目录那一行的内联提示（不是 live region —— live region 全弹窗只有一处） */
...
describe('工作目录下拉：不再有「默认工作目录」', () => {
    it('列表里只有现存工作区，没有「默认工作目录」，也没有空值项', async () => {
        render(<ScheduleEditModal initial={base} onSave={vi.fn()} onClose={vi.fn()}/>)
        await openWorkspaceDropdown()

        expect(screen.queryByText('默认工作目录')).toBeNull()
        const options = screen.getAllByRole('option')
        expect(options.map(o => o.textContent)).toEqual(['主工作区 (E:/ws1)', '备用工作区 (E:/ws2)'])
        // 当前值就是 ws-1：触发按钮上显示的是它，而不是兜底文案
        expect(screen.getByLabelText('工作目录').textContent).toContain('主工作区 (E:/ws1)')
    })

    it('未设置工作目录：触发按钮给出必选提示，列表里以禁用占位显示现状', async () => {
        render(<ScheduleEditModal initial={{...base, workspaceId: null}} onSave={vi.fn()} onClose={vi.fn()}/>)

        // 不静默留白：按钮上明说「未设置工作目录（必选）」
        expect(screen.getByLabelText('工作目录').textContent).toContain('未设置工作目录（必选）')

        await openWorkspaceDropdown()
        const placeholder = screen.getByRole('option', {name: '未设置工作目录'}) as HTMLButtonElement
    ...
    it('失效的 workspaceId：占位项显示原值并标注已失效，而不是显示为空', async () => {
        ...
        await waitFor(() => expect(screen.getByLabelText('工作目录').textContent)
            .toContain('工作目录已失效（原值：ws-a0b1dd77-不存在）'))
        expect(await inlineNoticeText()).toBe(WORKSPACE_INVALID_MESSAGE)

        await openWorkspaceDropdown()
        const placeholder = screen.getByRole('option', {name: /工作目录已失效/}) as HTMLButtonElement
        expect(placeholder.disabled).toBe(true)
        // 占位项点不动：它是「现状说明」，不是一条可选路径
        fireEvent.click(placeholder)
        expect(screen.getByLabelText('工作目录').textContent).toContain('ws-a0b1dd77-不存在')
    })
})

describe('保存校验：工作目录必填且必须解析得到现存工作区', () => {
    it('未设置工作目录 → 拦在弹窗内，给出原因，不关窗、不丢表单', async () => {
    ...
    it('工作目录已失效 → 拦下并说明是「失效」而不是「没选」', async () => {
        const onSave = vi.fn(async (_data: ScheduleFormData) => ({ok: true, data: null} as const))
    ...
    it('改成有效工作目录后保存成功：交给父层的是新工作区 id', async () => {
        const onSave = vi.fn(async (_data: ScheduleFormData) => ({ok: true, data: null} as const))
        render(<ScheduleEditModal initial={{...base, workspaceId: 'ws-a0b1dd77-不存在'}}
                                  onSave={onSave} onClose={vi.fn()}/>)
        ...
        // 换成有效工作区后，失效占位与提示都消失
        expect(screen.getByLabelText('工作目录').textContent).toContain('备用工作区 (E:/ws2)')
...
describe('列表未就绪不得放行，也不得冒充「已失效」（B1）', () => {
    it('取数失败 + **合法** id：说「无法校验」而不是「已失效」，保存被拦，并给出重试入口', async () => {
        ...
        expect(screen.queryByText(/工作目录已失效/)).toBeNull()
        expect(screen.getByLabelText('工作目录').textContent).not.toContain('已失效')
        ...
        const retry = screen.getByRole('button', {name: '重试加载工作区列表'})
        ...
        expect(screen.queryByRole('button', {name: '重试加载工作区列表'})).toBeNull()
        fireEvent.click(screen.getByText('保存'))
        await waitFor(() => expect(onSave).toHaveBeenCalledTimes(1))
    ...
    it('桥接缺失（window.electronAPI.workspace.list 不存在）：拦住 + 重试入口，不炸', async () => {
        ...
        expect(await clickSaveAndReadAlert()).toBe(WORKSPACE_LIST_UNREADY_MESSAGE)
        expect(onSave).not.toHaveBeenCalled()
        expect(screen.getByRole('button', {name: '重试加载工作区列表'})).toBeTruthy()
    })

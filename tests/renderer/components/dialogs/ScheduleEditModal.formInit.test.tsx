 ...
 *    → 父层任何一次重渲染都重跑回填，用户输入的名字与选好的项目被**静默还原**。
 *    本票新增的健康度广播又显著抬高了父层重渲染频次，于是「重新选一个可用项目」的动作
 *    会在下一次广播后被打回失效值 —— 功能在真实环境直接不可用。
 *  - **S1**：初值（新建时的「当前项目」）还没落地就点保存 → 报「请选择项目」
...
const workspaceText = () => screen.getByLabelText('项目').textContent ?? ''
...
describe('R9：父层重渲染不得重置表单', () => {
    it('父层以「同值但**新对象**」的 initial 重渲染后：已输入的名字与已选的项目都保留', async () => {
...
describe('S2：编辑存量任务不回填项目，新建保留默认', () => {
    it('编辑一条本来就没有项目的任务：不回填当前工作区，显式要求用户选', async () => {
        const onSave = vi.fn(async (_data: ScheduleFormData) => ({ok: true, data: null} as const))
        ...
        expect(workspaceText()).toContain('未设置项目（必选）')
        // 用户只改名字、完全没碰项目 → 保存必须被拦，而不是静默绑到 ws-2 上
        fireEvent.change(nameInput(), {target: {value: '只改名字'}})
        fireEvent.click(saveButton())
        expect((await screen.findByRole('alert')).textContent).toBe('请选择项目')
...
describe('S1：初值未就绪时保存按钮置灰，不报用户没做过的动作', () => {
    it('getCurrent() 未落地时：按钮禁用 + 等待态；落地后恢复可点并照常保存', async () => {
        ...
        // 未就绪：按钮点不动，也不会报「请选择项目」（用户没做过这个动作）
    ...
    it('编辑一条已有项目的任务：初值同步就绪，保存按钮立即可用（不引入多余等待）', () => {

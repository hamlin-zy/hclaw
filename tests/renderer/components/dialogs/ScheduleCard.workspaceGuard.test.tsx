...
/**
 * 定时任务行 · 工作目录失效标记与「立即执行」禁用（票 11 · workspace-guard）
 *
 ...
 *   - 换成可用工作目录后标记消失、按钮恢复可点（自动恢复，无需其他操作）。
 */
...
describe('工作目录失效标记', () => {
    it.each([
        ['missing', '工作目录已不存在', '工作目录失效'],
        ['unset', '未设置工作目录', '未设置工作目录'],
        ['unavailable', '工作目录不可用（E:/gone）', '工作目录不可用'],
    ...
    it('标记的底色/文字/边框/字号全部取自令牌（S4）：边框回到中性，字号 text-2xs', () => {
        renderCard({state: 'missing', path: null, reason: '工作目录已不存在'})
...
describe('「立即执行」按钮：不可用时禁用且原因可读出', () => {
    it('missing：按钮 disabled，title 与 aria-label 都真实承载原因', () => {
        ...
        expect(button.getAttribute('aria-label')).toBe('立即执行不可用：工作目录已不存在')
        expect(button.getAttribute('title')).toBe('立即执行不可用：工作目录已不存在')
        expect(screen.getByRole('button', {name: '立即执行不可用：工作目录已不存在'})).toBeTruthy()
    })

    it('unavailable：原因里带上具体路径', () => {
        renderCard({state: 'unavailable', path: 'E:/gone', reason: '工作目录不可用（E:/gone）'})
    ...
    it('取不到 reason 时也给出非空的可访问名（不给读屏软件一个空名字）', () => {
        ...
        expect(runButton().getAttribute('aria-label')).toBe('立即执行不可用：工作目录不可用')
    })
...
describe('改成有效工作目录后自动恢复', () => {
    it('同一条任务：不可用 → 可用，标记消失、按钮恢复可点', () => {
        const {rerender} = renderCard({state: 'missing', path: null, reason: '工作目录已不存在'})
 ...
 */
describe('工作目录失效时「停止」仍然可点（S3）', () => {
    it('运行中 + 工作目录失效：按钮是可点的「停止」，不是禁用的「立即执行不可用：…」', () => {
        const {unmount} = renderCard({state: 'missing', path: null, reason: '工作目录已不存在'})
        // 先确认「未运行 + 失效」确实被拦（对照组就在同一个用例里，避免断言恒真）
        expect(runButton().disabled).toBe(true)
        unmount()

        renderCard({state: 'missing', path: null, reason: '工作目录已不存在'}, {}, true)
    ...
    it('运行中 + 目录不可用（unavailable）：同样给「停止」', () => {
        renderCard({state: 'unavailable', path: 'E:/gone', reason: '工作目录不可用（E:/gone）'}, {}, true)

...
/**
 * PromptConfigDialog 侧边栏宽度拖拽 · 卸载清理对称性回归护栏
 *
 * 与 modelSchemeDialog.dragCleanup.test.tsx 同构：startResizing 在 document 上挂
 * mousemove/mouseup 并把 body.style.cursor 锁成 'col-resize'，但没有卸载兜底 →
 * 拖拽进行中卸载组件会残留 document 监听，且 body 内联光标永久停在 col-resize。
 * 本测试锁定「拖拽中卸载 = 监听移除 + body 光标复位」，与 MenuDialog 的 dragCleanupRef 范式对称。
 */
import {describe, it, expect, vi, afterEach} from 'vitest'
import {render, fireEvent, cleanup} from '@testing-library/react'
import PromptConfigDialog from '../../../../src/renderer/components/dialogs/PromptConfigDialog'
...
describe('PromptConfigDialog / 侧边栏宽度拖拽中的卸载清理', () => {
    it('拖拽中卸载 → 移除 mousemove/mouseup 监听并复位 body.cursor', () => {
        const {added, removed} = spyDocListeners()
        const {container, unmount} = render(<PromptConfigDialog/>)

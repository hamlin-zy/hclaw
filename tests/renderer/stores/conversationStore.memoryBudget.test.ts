 ...
 * - D1：`messagesMap` 用数量约束（预热 ≤10 + 驻留上限 20 + 切换时 LRU 淘汰）取代字节预算。
 *
...
describe('D1 — messagesMap 数量约束', () => {
    /** 把 n 个会话直接塞进 messagesMap，lastActiveAt 随索引递增（编号越大越新） */
    ...
    // ── a) 预热只加载最新 10 个会话 ──────────────────────
    describe('a) 启动预热只加载最近更新的 10 个会话', () => {
        beforeEach(() => {
        ...
        it(`预热只有 ${PRELOAD_MAX} 个会话进入 messagesMap，且是最近更新的那些`, async () => {
            ...
            // 预热为后台 IIFE（每批 5），等待其收敛
            await vi.waitFor(() => {
                expect(Object.keys(useConversationStore.getState().messagesMap).length)
                    .toBeGreaterThanOrEqual(PRELOAD_MAX + 1)
            }, {timeout: 3000})
            await new Promise(r => setTimeout(r, 100))

            const m = useConversationStore.getState().messagesMap
            const ids = Object.keys(m)
            // 激活会话（createdAt 最大）1 个 + 预热 10 个
            expect(ids.length).toBe(PRELOAD_MAX + 1)
            expect(m['conv-14']).toBeDefined()
            // 预热的是 updatedAt 最大的 10 个：conv-13 … conv-4
            for (let i = 4; i <= 13; i++) expect(m[`conv-${i}`]).toBeDefined()
            // 最旧的 4 个（conv-0 … conv-3）绝不预热
            for (let i = 0; i <= 3; i++) expect(m[`conv-${i}`]).toBeUndefined()
            // 预热进内存的会话必须登记进 LRU（可被 cleanupInactiveConversations 回收）
            const rendered = useConversationStore.getState().renderedConversationIds
            for (let i = 4; i <= 13; i++) expect(rendered).toContain(`conv-${i}`)
        })
    })

    // ── b) 连续切换后始终 ≤ 20 ──────────────────────────
    describe('b) 切换会话后 messagesMap 键数始终 ≤ 20', () => {
    ...
    describe('c) 超限驱逐最后激活时间最旧的会话，active/running/thinking 受保护', () => {
        it('被驱逐的是最久未激活者；active 与 running/thinking 会话保留', async () => {
            // 22 个普通会话 + 1 个 active = 23 → 需驱逐 3 个
            seed(22, {active: 'conv-active'})
            mockAgent.convAgentStates = {
                'conv-0': {agentState: {status: 'running'}},
                'conv-1': {agentState: {status: 'thinking'}},
            }

            await useConversationStore.getState().setActiveConversation('conv-active')

            const s = useConversationStore.getState()
            expect(Object.keys(s.messagesMap).length).toBe(MAP_MAX)
            // active 不被驱逐
            expect(s.messagesMap['conv-active']).toBeDefined()
            // running / thinking 不被驱逐（虽然它们是最旧的）
            expect(s.messagesMap['conv-0']).toBeDefined()
            expect(s.messagesMap['conv-1']).toBeDefined()
            // 最旧的 3 个非保护会话被驱逐：conv-2 / conv-3 / conv-4
            ...
            // 最近的会话保留
            expect(s.messagesMap['conv-21']).toBeDefined()
        })
    })
    ...
    describe('d) 驱逐会同步清理各会话级表，无悬空 id', () => {
        it('hasMoreMap / loadingMoreMap / conversationLastActiveAt / renderedConversationIds 同步清理', async () => {
            seed(20, {active: 'conv-active'}) // 20 + 1 = 21 → 驱逐 1 个（conv-0，最旧）
            useConversationStore.setState({

...
const startAgentCoreStub = vi.hoisted(() => vi.fn(
    async (_params: {conversationId: string; message: string; messageMetadata?: Record<string, unknown>}, _origin?: string) => undefined,
))
vi.mock('@/main/agent/startAgentCore', () => ({startAgentCore: startAgentCoreStub}))

/** 观察点：taskTarget 经同源解析器 resolveEntityCommand 决定前缀与 commandId 透传 */
const resolveEntityCommandStub = vi.hoisted(() => vi.fn<(name: string) => {template: string; commandId: string} | null>(() => ({template: '# 代理模式', commandId: 'agent:general-1'})))
vi.mock('@/main/agent/entityCommandResolver', () => ({resolveEntityCommand: resolveEntityCommandStub, resolveSkillCommand: vi.fn(() => null)}))
...
function recordOf(overrides: Partial<ScheduleRecord>): ScheduleRecord {
    return {
        ...
        runCount: 0, createdAt: 1, updatedAt: 1, workspaceId: 'ws-fixture', isSystem: false,
        ...overrides,
    }
}

/** 跑一次 agent 任务，返回交给 startAgentCore 的完整参数（message / messageMetadata） */
async function runAgentTask(taskTarget: string, taskArgs: any[]): Promise<{message: string; messageMetadata?: Record<string, unknown>}> {
    ...
    const call = startAgentCoreStub.mock.calls[0]?.[0] as {message: string; messageMetadata?: Record<string, unknown>} | undefined
...
beforeEach(() => {
    vi.clearAllMocks()
    resolveEntityCommandStub.mockReturnValue({template: '# 代理模式', commandId: 'agent:general-1'})
...
describe('scheduler.buildUserMessage：换行分隔（能力名 / 正文）', () => {
    it('正文以 Markdown 标题开头时，标题自成一行的下一行，不被并进第一行', async () => {
        ...
        const msg = (await runAgentTask('General', [prompt])).message
    ...
    it('单行正文同样用换行分隔', async () => {
        expect((await runAgentTask('General', ['同步工作日志'])).message).toBe('/General\n同步工作日志')
    })

    it('正文前后的空白被 trim，分隔符仍是单个换行', async () => {
        expect((await runAgentTask('General', ['  \n## 步骤\n  '])).message).toBe('/General\n## 步骤')
    })
    ...
    it.each([
        ...
        const msg = (await runAgentTask('General', args as any[])).message
        expect(msg).toBe('/General')
        expect(msg).not.toMatch(/\s$/)
    })
})

describe('scheduler.buildUserMessage：同源能力解析（方案 A）', () => {
    it('taskTarget 命中注册能力 → 前缀照拼 + 透传 messageMetadata.commandId', async () => {
        resolveEntityCommandStub.mockReturnValue({template: '# 代理模式: General Agent', commandId: 'agent:general-1'})
        const r = await runAgentTask('General', ['## 任务目标\n同步日志'])
        expect(r.message).toBe('/General\n## 任务目标\n同步日志')
        expect(r.messageMetadata).toEqual({commandId: 'agent:general-1'})
    })

    it('taskTarget 未命中解析器 → 纯正文、无 / 前缀、不透传 commandId', async () => {
        resolveEntityCommandStub.mockReturnValue(null)
        const r = await runAgentTask('General', ['## 任务目标\n同步日志'])
        expect(r.message).toBe('## 任务目标\n同步日志')
        expect(r.messageMetadata).toBeUndefined()
    })

    it('解析器抛错 → 降级为不拼前缀的纯正文（与 memo 链路同源），不透传 commandId', async () => {
        resolveEntityCommandStub.mockImplementation(() => { throw new Error('resolver boom') })
        const r = await runAgentTask('General', ['同步日志'])
        expect(r.message).toBe('同步日志')
        expect(r.messageMetadata).toBeUndefined()
    })

    it('命中且 prompt 为空 → 只留下 /能力，且透传 commandId', async () => {
        const r = await runAgentTask('General', [])
        expect(r.message).toBe('/General')
        expect(r.messageMetadata).toEqual({commandId: 'agent:general-1'})
    })
})

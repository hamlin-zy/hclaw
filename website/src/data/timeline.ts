...
export const timelineItems: TimelineItem[] = [
  {
    version: 'v0.5.18', date: '2026-09-20', tag: '最新',
    title: '会话跳转与稳定性修复',
    items: ['子会话窗口支持「加载更多」，运行中会话一键直达对应视图', '修复输出截断后同会话后续命令结果偶发为空的问题', '应用退出时正确回收文件检索子进程，不再残留后台进程'],
  },
  {
    version: 'v0.5.17', date: '2026-09-19',
    title: '用户习惯记忆系统与系统任务保护',
    items: ['用户习惯记忆系统：自动沉淀偏好与经验，会话中注入，越用越懂你', '系统任务保护与一键还原，配置漂移自动检测', 'OpenRouter 固定服务商单选锁死，弃自动降级', '修复侧栏宽度持久化、会话归属显示、全局方案服务商判定等问题'],
  },
  {
    version: 'v0.5.16', date: '2026-09-18',
    title: '项目分组管理与设置页改版',
    items: ['项目分组管理：侧栏按组展示会话，切换项目组一目了然', '设置页拆分为标签页，查找更直观', '定时任务新增脚本日志与执行状态追踪', '侧栏可拖拽调宽并持久化，会话列表分段展示'],
  },
  {
    version: 'v0.5.15', date: '2026-09-15',
    title: '冷启动提速与能力页统一',
    items: ['冷启动更快、首屏更早出现，技能扫描与 MCP 启动优化', 'Agent、技能、命令、插件四个管理页统一改版', '项目窗口新增快速打开浮层与键盘操作', '修复冷启动后命令管理页显示为空等问题'],
  },
  {
    version: 'v0.5.14', date: '2026-09-14',
    title: '本地图片识别与界面统一',
    items: ['本地图片可直接交给 AI 识别，图片自动压缩、质量可调', '项目窗口三列可拖动标题栏换序，面板样式与主界面统一', 'MCP 扩展连接与版本管理加固，工具调用消息合并为可展开卡片', '修复文件树偶发显示为空、窗口四周 1px 白边等问题'],
  },
  {
    version: 'v0.5.13', date: '2026-09-12',
    title: '项目窗口与 Git 可视化',
  ...
  {
    version: 'v0.5.10', date: '2026-09-07',
    title: '备忘录与会话列表日期分组',
    items: ['备忘录面板拆分为置顶/全部双 Tab，历史记录按日期分组折叠，支持拖拽排序', '会话列表按日期分组：置顶脱离、今天平铺、历史折叠', '修复模型价格未配置时误显示为 0、极简模式工具调用组样式错位'],
  },
  {
    version: 'v0.5.9', date: '2026-09-06',
    title: '模型详情弹窗与运行时参数',
    items: ['模型价格、类型与 per-model 运行时参数集中到详情弹窗配置', '会话交接后待办任务不丢失，重启应用依然保留', '价格录入所见即所得，混合币种自动标注币种'],
  },
  {
    version: 'v0.5.8', date: '2026-09-06',
    title: 'MCP 服务器版本管理',
    items: ['支持查看与切换 MCP 服务器历史版本，一键回退稳定版本', '插件服务器升级进度与结果直观展示', '会话上下文用量展示口径统一'],
  },
  {
    version: 'v0.5.7', date: '2026-09-05',
    title: '快捷短语功能上线',
  ...
  {
    version: 'v0.5.2', date: '2026-09-01',
    title: 'LLM 追踪与缓存稳定性修复',
    items: ['修复长对话中 KV cache 偶发断裂的问题，减少重复计算与费用浪费', 'LLM 调用记录按对话轮次正确归类，同轮多段思考内容完整拼接展示', '追踪列表支持一键复制会话运行时路径，排查问题更方便'],
  },
  {
    version: 'v0.5.1', date: '2026-08-31',
    title: '界面元素统一标识与模型选择优化',
    items: ['全部可交互元素补齐统一组件标识，配套自动校验规则与守护测试，为自然语言指令操控界面打基础', '子会话默认使用轻量模型，主会话仍走主力模型，角色降级链按需自动切换', '模型选择与性能成本更平衡'],
  },
  {
    version: 'v0.5.0', date: '2026-08-31',
    title: '备忘录功能与用量过滤',
    items: ['全新备忘录面板：搜索、置顶排序、能力绑定，支持独立编辑窗口与拖拽上传附件', '备忘录一键发送到会话处理，图片附件直接预览', '会话增删跨窗口实时同步，会话交接链路支持附件传递', '用量明细新增时间/服务商过滤，含「昨天」快捷范围；下拉选择器全面主题化'],
  },
  {
    version: 'v0.4.16', date: '2026-08-29',

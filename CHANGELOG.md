...
### 新增
- **运行中会话快速跳转** — 子会话窗口支持「加载更多」，点击运行中的会话可直达对应视图，不再迷失在长列表中 (`ConversationSidebar` / `runningSessionsJump`)

### 修复
- **长输出命令结果偶发为空** — 输出被截断后，同一会话的后续命令可能收不到任何输出的问题 (`shellPool`)
- **退出后残留后台进程** — 应用退出时正确回收文件检索（rg）子进程，不再遗留后台进程占用资源 (`project-manager/search`)

## [v0.5.17] - 2026-09-19

### 新增
- **用户习惯记忆系统** — HClaw 会在使用中自动沉淀你的项目偏好与经验知识（每小时定时整理，带冷却期与质量门槛防止脏知识），并在后续会话中自动注入，越用越懂你 (`memory/` / `memoryPublish` / `systemSchedules`)
- **系统任务保护与一键还原** — 定时任务中的系统任务（如记忆沉淀）不可误改误删，配置偏离出厂时自动检测并支持一键还原 (`ScheduleSystemActions` / `scheduleOps`)
- **OpenRouter 固定服务商** — 可锁定单一服务商并禁用自动降级，避免请求被路由到不稳定或高成本节点 (`openRouterProviders` / `openaiAdapter`)

### 修复
- **侧栏宽度持久化失效** — 拖拽调整的侧栏宽度重启后恢复默认的问题 (`sidebarStore`)
- **会话归属显示异常** — 部分场景下会话列表出现 `__unassigned__` 字样、跨段定位错误、实时事件不刷新等问题 (`conversationStore` / `workspacePath`)
- **全局方案服务商判定失效** — 全局模型方案下按 baseUrl 判定服务商的逻辑失效 (`model/index.ts`)
- **文件检索排除规则** — File Search 不再受 .gitignore 影响，改用明文排除清单，检索结果更符合预期 (`EXCLUDED_FILE_GLOBS`)
- **模型方案自动选中** — 打开方案页未自动选中当前激活方案的问题 (`ModelSchemeDialog`)
- **界面细节** — Markdown 表格过窄改为横向滚动、弹窗配色统一为主题 token、定时任务自定义模式回显 cron 表达式等

### 其他
- 定时任务健康度感知系统任务，不再误报不可用；记忆沉淀任务改为每小时执行
...
### 新增
- **能力管理四个页面统一改版** — Agent、技能、命令、插件四个管理页统一界面与交互，共享卡片、分组与状态样式，操作更一致直观 (`Modal` / `CapabilityCard` / `PluginGroupCard` / 四个管理页)
- **项目窗口新增快速打开浮层** — 项目窗口加入 QuickOpen 快捷搜索浮层与完整键位支持，纯键盘操作更顺畅 (`QuickOpen` / PM 键位层)
- **Agent 到达轮数上限有明确提示** — 单次会话达到最大轮数时以独立状态收尾并提示，不再静默中断 (`TurnLimitNotice`)

### 变更
- **冷启动更快、首屏更早出现** — 窗口优先显示、托盘延后加载，消除能力扫描对首帧的阻塞，启动过程分段可观测 (`main/index.ts` / `startupTrace` / `initProgress`)
- **技能扫描与 MCP 启动优化** — 技能收集与 MCP 服务启动更省时，插件服务器加载更稳定 (`skills/loader` / `mcp/client` / `mcpWorker`)
- **工具名纠偏加强** — 工具名称纠错覆盖更多场景，遇到歧义时明确拒绝而非误调用别名 (`toolNameResolver`)
- **稳定性与内存加固** — Agent 子进程设置内存上限，长会话内存占用可控；插件归属判定收敛到统一解析器，命令重名不再静默覆盖 (`workerLimits` / `pluginOwnership`)

### 修复
- **修复冷启动后命令管理页显示为空** — 启动阶段补齐能力投影，打开命令管理页即可正常看到数据 (`powerManager`)
- **修复复杂对话场景下的偶发报错** — 会话切换竞态、消息重复水合与工具调用异常处理加固，长会话更稳 (`messageUtils` / `useResetTimeout`)
...
### 新增
- **本地图片可直接交给 AI 看** — 提到某个本地图片文件，AI 就能加载并识别内容，图片自动压缩以控制上下文开销，图片质量可在设置中调节 (`loadImageTool` / `imageCompress`)
- **项目窗口上半区三列可拖动换序** — 拖动面板标题栏即可调整三列排列，下次打开仍保持 (`usePaneReorder` / `paneOrder`)
- **内置 Agent 支持随版本升级** — 内置 Agent 提示词可自动升级到新版，无需手动重建 (`builtinAgentUpgrade`)

### 变更
- **MCP 扩展能力整体加固** — 连接、版本管理与插件服务器加载更稳定，服务器命名更清晰不混淆 (`mcp/client` / `pluginServers` / `shared/mcp/naming`)
- **工具调用消息展示优化** — 同类工具调用合并为可展开卡片，长会话滚动更清爽 (`displaySegments` / `compact-popup`)
- **项目窗口面板恢复圆角卡片样式** — 面板描边与阴影统一，观感更贴合主界面 (`globals.css`)
- **界面字体与图标成套更新** — 全局换用新字体与图标体系，并加入对比度、弱化文本的自动检查，界面更统一也更易读 (`styles/globals.css` / `icons` / `audit-contrast`)

### 修复
- **修复项目窗口文件树偶发显示为空** — 缓存缺失时不再假装"没有文件"，加载中会显示加载动画 (`FileTree` / `fileTreeStore`)
- **修复窗口四周出现 1px 白边** — 背景层覆盖窗口描边，界面边缘更干净 (`App.tsx`)
- **修复会话交接时技能名被误当命令注入** — 未命中的能力名不再静默注入，交接内容更干净 (`sessionHandoffTool`)
...
### 新增
- **项目窗口（Project Manager）** — 全新独立窗口，把文件树、代码编辑器、Git 面板装进同一个工作台，不用离开应用就能读代码、改代码 (`project-manager/`)
- **Git 可视化面板** — 文件改动一目了然，分支树支持本地/远程分层与搜索，提交列表可展开详情，差异支持并排对比与逐处跳转，还能在应用内直接提交和推送 (`GitStatusPanel` / `GitBranchTree` / `DiffViewer`)
- **编辑器与预览** — 内置语法高亮编辑器（可用 Vim 键位）、Markdown 实时预览与分屏、图片查看 (`CodeEditor` / `MarkdownPreview`)
- **发送到会话** — 把文件、改动、提交或选中的代码行直接发给 AI，看代码时随手就能提问 (`sendToConversation`)
- **Agent 可显式调用 MCP 工具** — 新增工具让 AI 直接调用各类扩展能力，过程可见 (`call_mcp_tool`)

### 变更
- **命令执行明显更快** — 改用常驻 Shell 会话，省去每条命令重复启动的开销 (`shellPool`)
- **代码搜索更准更快** — 搜索底层改用 ripgrep，修好正则与大文件匹配问题 (`grepTool` / `globTool`)
- **AI 改动工具集前会先征求同意** — 不再中途静默变更可用工具，可清楚掌控 AI 能做什么 (`toolsChangeConfirm`)
- **长会话更省内存** — 长时间运行的会话内存占用有上限，历史工具结果自动收缩，久聊不易变卡
- **文件改动的预览更好读** — 编辑文件时按窗口展示前后差异 (`fileEditTool`)
- **编辑器相关依赖升级** — 新增 CodeMirror、chokidar、ripgrep 等依赖

### 修复
- **修复复杂任务中途偶发中断** — 阻塞状态可正常恢复，多个子 Agent 并行时权限不再互相干扰
- **修复外部改动文件后内容不同步** — 文件被外部修改后编辑器会自动刷新，不再覆盖丢失内容
- **修复命令名校验漏洞** — 拦截 Windows 保留设备名与非法字符 (`commandName`)
...
### 新增
- **Agent 可直接查询数据库** — 新增只读数据库查询工具，可检索历史会话、任务与用量统计，带 SQL 安全防护 (`hclaw_db_query`)
- **备忘录体系升级** — 备忘录支持优先级标注与排序；Agent 可代为创建/管理备忘录，AI 主动帮你记事更可靠 (`memo_tool` / `PrioritySelect.tsx`)
- **Agent 工具白名单** — 可为 Agent 指定可用的工具集合，服务商支持自定义请求头，兼容性更灵活 (`PromptConfigDialog.tsx`)
- **历史组一键批量删除** — 在历史分组标题上右键即可删除整组备忘录 (`ConversationSidebar.tsx`)
- **用量统计筛选增强** — 用量窗口支持时间范围筛选，新增本周/本月快捷统计 (`UsageWindow.tsx`)
- **用户消息复制按钮** — 一键复制用户消息，成功后有提示反馈 (`MessageItem.tsx`)
- **配置驱动的快捷键系统** — 快捷键改为配置驱动，扩展更方便 (`shortcuts`)

### 变更
- **性能大幅提升** — 用量统计查询提速约 3.6 倍；Agent 名册输出 token 开销降低约 2/3 (`llmUsageRepository.ts` / `list_agents`)
- **界面细节打磨** — About 对话框调整、Agent 预览支持 Markdown 渲染、附件预览缩略图与品牌色边框

### 修复
- **修复后台会话子 Agent 事件偶发丢失** — 复杂任务不再整批丢弃事件，运行中卡片可跳转子会话查看进度 (`sessionManager`)
- **修复工具调用偶发失败** — 提升复杂场景下工具调用成功率
- **修复备忘录拖拽松手误开编辑窗口** — 拖拽排序后不再误触编辑 (`MemoPanel.tsx`)
...
### 新增
- **快捷键设置页补全** — 补齐缺失的快捷键配置项，修正 macOS 平台快捷键说明，跨平台展示更准确 (`SettingsDialog.tsx` / `Kbd.tsx`)
- **工作目录卡片显示 git 分支徽章** — 工作目录卡片直观展示当前 git 分支（只读感知），悬停显示更多信息，徽章主题随界面配色 (`gitBranch.ts` / `App.tsx`)
- **跨天自动刷新日期分组** — 会话与备忘录列表跨天后自动按新日期重新分组，无需手动刷新 (`useDayBoundaryTick.ts`)

### 变更
- **请求头兼容性改进** — 注入符合 OpenCode Go 规范的请求头，提升与部分服务商的兼容性 (`opencodeHeaders.ts`)

### 修复
- **修复价格全为 0 时红点提示不显示** — 模型价格未正确配置时的红点提醒恢复显示 (`modelParams.ts` / `ConversationSidebar.tsx`)
...
### 新增
- **备忘录双 Tab 列表与历史日期分组** — 备忘录面板拆分为「置顶 / 全部」双 Tab，历史备忘录按日期分组折叠，支持拖拽重新排序并带过渡动画 (`MemoPanel.tsx` / `memoSort.ts`)
- **会话列表日期分组** — 会话列表按日期分组展示：置顶会话脱离分组、今天的会话平铺、历史会话按日期层级折叠 (`ConversationSidebar.tsx`)

### 修复
- **修复模型价格未配置时被误显示为 0 的问题** — 价格未命中返回未配置（undefined）而非 0，与「免费」正确区分，模型详情橙点标识判定更准确 (`modelParams.ts` / `modelMetaRegistry.ts`)
- **修复极简模式下工具调用组概览样式错位** — 概要行圆点与展开详情按钮恢复垂直居中 (`ToolCallRenderer.tsx`)
...
### 新增
- **模型详情弹窗** — 服务商编辑弹窗的模型表新增 ⚙️ 详情入口，承载价格 ×4、模型类型多选与 per-model 运行时参数（最大上下文 / 温度 / 最大输出）配置；有自定义参数的模型显示橙色徽标 (`ModelDetailModal.tsx` / `ModelTable.tsx`)
- **per-model 运行时参数** — 每个模型可单独配置最大上下文、温度、最大输出，运行时按「自定义 → OpenRouter → 系统设置/兜底」统一优先级链解析生效，解析收敛到单一入口 `src/shared/modelParams.ts`；handoff 阈值、请求参数、用量窗口同口径
- **留空即兜底交互** — 弹窗内输入框留空 = 不落库，默认值以 placeholder 展示（OpenRouter 来源青虚线 / 系统设置来源灰实线）；仅用户填写值落库，非法值标红并禁用确定 (`modelDetailCommit.ts`)
- **会话交接后待办不丢失** — 会话交接（handoff）时，来源会话的活跃待办会自动转移到新会话，切换会话或重启应用后待办依然保留 (`sessionHandoffTool.ts` / `taskBatchRepository.ts`)

### 变更
- **模型表瘦身 8 → 5 列** — 价格 4 列与填充列迁入模型详情弹窗，主表保留模型 ID / 类型 / 详情 / 测试 / 列表操作，窄窗口不再横向滚动
- **价格录入所见即所得** — 模型价格按录入的币种原样回显，不再因汇率换算产生数值漂移；混合币种价格单元格会标注币种 (`priceEditing.ts`)

### 优化
- **界面动画统一** — 28 个组件的进出场动画收敛到统一预设，收窄全局过渡样式，减少不必要的界面重绘 (`motionPresets.ts`)
...
### 新增
- **快捷短语功能** — 支持自定义常用短语，通过 Ctrl+Shift+V 或短语选择弹窗一键插入，并可在短语管理窗口增删改查与排序

### 修复
- **修复历史消息中偶发出现系统内部提示的问题** — 过滤缺少来源标记的隐式系统消息，避免污染对话历史
- **修复插件仓库版本号污染「仓库」标签红点的问题**
- **修复终止任务后残留空回复气泡的问题**

### 变更
- 任务正常完成时，任务栏/托盘图标会闪烁提醒，无需一直盯着窗口
...
### 新增
- **思考强度新增「禁用」档位** — 现在可以在会话中彻底关闭模型的思考过程，按各家模型供应商的语义发送关闭参数，响应更快 (`ThinkingEffortSelector.tsx` / `thinkingEffort.ts`)

### 变更
- **指令消息直接落库展示** — 指令模板（command-task）消息改为在主循环前真实写入并保存，不再走合成注入与重放机制，长对话中指令消息的显示与回放更可靠 (`controller.ts` / `conversationRepository.ts`)
- **技能列表变更改为追加式更新** — 对话中的技能列表更新以追加新消息的方式呈现，不再原地替换旧消息，历史记录更清晰完整 (`catalogPublish.ts`)
- **稳定性与内部结构优化** — 清理消息注入与历史重建的冗余逻辑，同毫秒消息按插入顺序稳定返回，修复个别场景下消息顺序不确定的问题 (`userContentBuilder.ts` / `conversationRepository.ts`)
...
### 新增
- **界面元素统一标识** — 为「用自然语言指令操控界面」打下基础：全部可交互元素补齐统一组件标识，并配套自动校验规则与守护测试，确保标识不缺失、不重名 (`eslint-rules/data-name-unique.ts` / `dataNameGuard.test.tsx`)

### 变更
- **子会话模型选择更智能** — 子会话默认使用轻量模型，主会话仍走主力模型；模型角色降级链按需自动切换，性能与成本更平衡 (`modelSelector.ts` / `setup.ts`)
- **复制提示组件化** — 复制成功提示抽为共享组件，界面各处复用更一致 (`common/CopyToast.tsx`)
...
### 新增
- **Agent 循环检测机制** — 自动识别 Agent 陷入重复操作的卡死循环，触发时通过警告横幅提醒或自动暂停，避免无意义消耗 token（`loopDetector.ts` / `LoopWarningBanner.tsx`）
- **循环检测设置** — 设置窗口可配置检测阈值与触发模式（仅提醒/提醒并暂停），按需调整灵敏度（`SettingsDialog.tsx`）
- **任务栏闪烁提醒** — 触发循环警告时任务栏图标闪烁，即使窗口最小化也不会错过提醒（`attention.ts`）
- **更新检查 Gitee 兜底** — GitHub 版本检查失败时自动改用 Gitee 源检查更新，国内网络环境下更新提醒更可靠（`updateChecker.ts`）
- **技能工具白名单运行时校验** — Agent 运行中的技能工具白名单实时生效，修改后无需重启即可应用（`capabilityManager.ts`）

### 修复
- **模型用量解析兼容性** — 兼容智谱等返回格式的用量解析，统计数据不再缺失（`llmUsageParser.ts`）
- **警告横幅显示优化** — 循环警告横幅关闭后重复指纹不再反复弹出，升级提醒可再次展示（`LoopWarningBanner.tsx`）
- **下拉选择器体验** — 主题化下拉组件统一风格，弹窗内滚动时不再错位消失（`ThemedSelect.tsx`）
...
### 新增
- **服务商编辑窗口全新改版** — 三段式布局重新组织服务商配置，Google 双认证支持独立布局，信息分区更清晰（`ProviderEditModal.tsx`）
- **模型价格管理** — 服务商编辑窗口内新增模型管理表格，可查看和编辑各模型输入/输出/缓存价格，支持从 OpenRouter 元数据一键拉取填充，也支持逐格手填（`ModelTable.tsx`）
- **多币种切换** — 价格编辑支持 CNY/USD 等币种切换，切换时自动换算已填价格，默认币种跟随设置（`priceEditing.ts`）
- **用量按服务商精确计价** — 用量记录新增服务商归属，成本重算时优先使用该服务商的自定义价格，用量明细表新增价格来源说明与排序（`usagePriceResolver.ts` / `UsageWindow.tsx`）

### 变更
- **模型类型自动识别** — 拉取模型列表后自动根据 OpenRouter 元数据识别模型类型（对话/视觉/推理等），减少手动配置（`modelMetaRegistry.ts`）

### 修复
- **弹窗体验优化** — 服务商编辑窗口内的模型测试错误提示、下拉菜单在滚动表格中不再错位消失；原生 confirm/alert 全部替换为应用内确认弹窗与轻提示，界面风格统一（`ModelTable.tsx` / `ConfirmDialog.tsx`）
- **暗色主题显示修正** — 模型表格表头与币种切换在暗色主题下的配色修正（`ModelTable.tsx`）
...
### 新增
- **LLM 日志窗口大幅增强** — 新增工具调用明细页签、响应原文/正文双视图与"概览/请求体"拆分展示；多次重试合并为单张卡片、可逐次展开查看；详情面板可拖拽调高，排查模型调用更直观（`LlmLogsWindow.tsx` / `TimelineView.tsx`）
- **日志中直接显示会话标题** — LLM 日志时间线与会话下拉按最新记录排序，并直接显示会话库标题，快速定位目标日志（`llmTraceRecorder.ts`）
- **JSON 查看器增强** — 支持一键全部折叠/展开，内嵌长字符串中的 JSON 可展开为嵌套树浏览（`JsonTree.tsx`）
- **汇率与价目表手动刷新** — 用量窗口可一键手动刷新实时汇率与服务商价目表，完成后有即时提示反馈（`UsageWindow.tsx`）

### 变更
- **成本显示精度提升** — 成本统一显示为 5 位小数并降低最小显示阈值，新增"综合价格/M"列，小额调用的花费也一目了然（`format.ts` / `statsParts.tsx`）
- **自定义服务商地址兼容性更好** — 不再自动补全 `/v1` 路径，由地址本身决定完整路径，智谱 v4 等非标准路径的服务商可直接使用（`openaiAdapter.ts`）

### 修复
- **模型调用归因更准确** — 复杂对话场景下调用来源（主链路/子代理/后台）归属判断修正，步骤计数反映真实轮次，HTTP 状态码如实记录便于排障（`execute.ts` / `llmTraceRecorder.ts`）
